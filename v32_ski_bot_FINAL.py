...
# ═══════════════════════════════════════════════════════════════════════════
# SKI BOT - COLORADO MOUNTAINS CHECK-IN SYSTEM
# ═══════════════════════════════════════════════════════════════════════════
# Features:
# - User profiles with season passes (Epic, Ikon, Loveland)
# - Mountain check-in/checkout with timestamps
# - Weather updates and MOTD
# - Active riders announcements (toggle-able)
...
# ─────────────────────────────────────────────────────────────────────────
# List Commands (Show Active Riders)
# ─────────────────────────────────────────────────────────────────────────
...
async def admin_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    admin_commands_text = (
        ...
        "/forceMOTD - Force post MOTD to group now\n"
        "/weatherUpdate - Update weather data\n\n"
        "**Information:**\n"
...
async def force_motd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin command to force post MOTD immediately"""
    if update.message.chat.type != 'private':
        return ConversationHandler.END
    ...
    keyboard = []
    for answer in all_answers:
        keyboard.append([InlineKeyboardButton(str(answer), callback_data=f'force_motd_auth_{answer}')])
    ...
    return FORCE_MOTD_AUTH

async def force_motd_auth_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle auth response for forceMOTD"""
    query = update.callback_query
    await query.answer()
    
    answer = query.data.replace('force_motd_auth_', '')
    
    if answer == '13':
        await query.edit_message_text("✅ Authentication successful! Posting MOTD now...")
        
        # Get the GROUP_CHAT_ID from the main function (we'll pass it via context)
        GROUP_CHAT_ID = -1001090200926  # Same as in scheduler
        ...
        try:
            ...
            message = (
                ...
                f"💫 {motd_text}\n\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
                f"🏔️ Make sure you /addProfile once then you can use me!\n"
                f"Don't forget to use /skiing or /boarding to check in when you hit the slopes!\n\n"
                f"🌡️ Check out current temps by using /temp or /weatherUpdate\n\n"
                f"Use /ikon , /epic , /loveland to find out who is skiing where on that pass!\n"
                f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            )
            
            # Send to group
            result = await context.bot.send_message(chat_id=GROUP_CHAT_ID, text=message)
            
            # Confirm to admin
            await context.bot.send_message(
                chat_id=update.effective_user.id,
                text=f"✅ MOTD posted successfully to group!\nMessage ID: {result.message_id}"
            )
            
            logger.info(f"Admin {update.effective_user.id} forced MOTD post - Message ID: {result.message_id}")
            
        except Exception as e:
            error_msg = f"❌ Error posting MOTD: {e}"
            await context.bot.send_message(chat_id=update.effective_user.id, text=error_msg)
            logger.error(f"Error in force_motd: {e}")
...
async def daily_reset(context: ContextTypes.DEFAULT_TYPE):
    """Force EVERYONE to leave at 11:00 PM MT - No exceptions"""
    logger.info("=== DAILY RESET STARTED AT 11:00 PM MT ===")
    ...
    try:
        ...
        # Force checkout every single user
        for user_id, user_data in users.items():
            current_mountain = user_data.get('activeMountain', '')
            
            if current_mountain:
                username = user_data.get('username', 'Unknown')
                users_checked_out.append(f"{username} from {current_mountain}")
                checkout_count += 1
            
            # Force clear the mountain and timestamp
            user_data['activeMountain'] = ''
            user_data['checkin_timestamp'] = ''
        
        # Write all changes back to database
        write_users(users)
        
        if checkout_count > 0:
            logger.info(f"DAILY RESET: Forced checkout {checkout_count} users:")
            for user_info in users_checked_out:
                logger.info(f"  - {user_info}")
        else:
            logger.info("DAILY RESET: No users were checked in")
        
        logger.info("=== DAILY RESET COMPLETED - ALL MOUNTAINS CLEARED ===")
        
    except Exception as e:
        logger.error(f"ERROR in daily_reset: {e}")
        # Even if there's an error, try to force clear everyone
        try:
            users = read_users()
            for user_id in users:
                users[user_id]['activeMountain'] = ''
                users[user_id]['checkin_timestamp'] = ''
            write_users(users)
            logger.info("Emergency force clear completed despite error")
        except Exception as emergency_error:
            logger.error(f"CRITICAL ERROR in emergency force clear: {emergency_error}")

async def bot_conversation_cleanup(context: ContextTypes.DEFAULT_TYPE):
    """Clean up conversation states at 1:00 AM MT to prevent /addProfile from getting stuck"""
    logger.info("Bot conversation cleanup started at 1:00 AM MT")
    
    # Get the application from context
    application = context.application
    
    # Clear all conversation handlers' states
    for handler in application.handlers.get(0, []):
        if isinstance(handler, ConversationHandler):
            # Clear all conversations for this handler
            handler._conversations.clear()
            logger.info(f"Cleared conversations for handler: {handler.name if hasattr(handler, 'name') else 'unnamed'}")
    
    logger.info("Bot conversation cleanup completed - all conversation states cleared")
...
# ─────────────────────────────────────────────────────────────────────────
# Content Posting Jobs (MOTD, Weather, Active Riders)
# ─────────────────────────────────────────────────────────────────────────
...
async def message_of_the_day(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        logger.info("=== MOTD FUNCTION STARTED ===")
        
        motd_text = read_motd()
        logger.info(f"MOTD text loaded: {motd_text[:50]}...")
        ...
        logger.info(f"Attempting to send MOTD to chat_id: {chat_id}")
        
        result = await context.bot.send_message(chat_id=chat_id, text=message)
        
        logger.info(f"✅ MOTD POSTED SUCCESSFULLY to chat {chat_id} - Message ID: {result.message_id}")
        
    except Exception as e:
        logger.error(f"❌ ERROR posting MOTD: {e}")
        logger.error(f"Chat ID attempted: {context.job.data if context.job else 'No job data'}")
        import traceback
...
async def daily_weather_update(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        logger.info("=== WEATHER UPDATE FUNCTION STARTED ===")
        ...
        write_weather(weather_data)
        logger.info("Weather data written to database")
        ...
        chat_id = context.job.data
        logger.info(f"Attempting to send Weather Update to chat_id: {chat_id}")
        
        result = await context.bot.send_message(chat_id=chat_id, text=message)
        
        logger.info(f"✅ WEATHER UPDATE POSTED SUCCESSFULLY to chat {chat_id} - Message ID: {result.message_id}")
        
    except Exception as e:
        logger.error(f"❌ ERROR posting Weather Update: {e}")
        logger.error(f"Chat ID attempted: {context.job.data if context.job else 'No job data'}")
        import traceback
        logger.error(traceback.format_exc())
...
async def auto_announce(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        logger.info("=== AUTO ANNOUNCE FUNCTION STARTED ===")
        ...
        logger.info("Toggle is ON - proceeding with announcement")
        
        users = read_users()
        
        if not users:
            logger.info("No users in database - skipping announcement")
            return
        ...
        if not epic_active and not ikon_active and not loveland_active:
            logger.info("No active riders - skipping announcement")
            return  # Don't post if no one is on mountains
        
        logger.info(f"Active riders found - Epic: {len(epic_active)}, Ikon: {len(ikon_active)}, Loveland: {len(loveland_active)}")
        ...
        chat_id = context.job.data
        logger.info(f"Attempting to send Active Riders to chat_id: {chat_id}")
        
        result = await context.bot.send_message(chat_id=chat_id, text=message)
        
        logger.info(f"✅ ACTIVE RIDERS POSTED SUCCESSFULLY to chat {chat_id} - Message ID: {result.message_id}")
        
    except Exception as e:
        logger.error(f"❌ ERROR posting Active Riders: {e}")
        logger.error(f"Chat ID attempted: {context.job.data if context.job else 'No job data'}")
        import traceback
        logger.error(traceback.format_exc())
...
def main():
    ...
    force_motd_handler = ConversationHandler(
        entry_points=[CommandHandler('forceMOTD', force_motd)],
        states={
            FORCE_MOTD_AUTH: [CallbackQueryHandler(force_motd_auth_callback, pattern='^force_motd_auth_')],
        },
        fallbacks=[CommandHandler('cancel', cancel)],
        per_message=False,
        per_chat=False,
    ...
    application.add_handler(force_motd_handler)
    ...
    if job_queue:
        ...
        reset_time = time(hour=23, minute=0, tzinfo=mountain_tz)  # 11:00 PM
        snapshot_time = time(hour=16, minute=45, tzinfo=mountain_tz)
        motd_time = time(hour=4, minute=0, tzinfo=mountain_tz)
        bot_cleanup_time = time(hour=1, minute=0, tzinfo=mountain_tz)
        announce_430am = time(hour=4, minute=30, tzinfo=mountain_tz)
        announce_7am = time(hour=7, minute=0, tzinfo=mountain_tz)
        
        GROUP_CHAT_ID = -1001090200926
        logger.info(f"═══════════════════════════════════════════════════════")
        logger.info(f"CONFIGURED GROUP CHAT ID: {GROUP_CHAT_ID}")
        logger.info(f"All scheduled messages will be sent to this chat")
        logger.info(f"═══════════════════════════════════════════════════════")
        
        # ============================================
        # DAILY MAINTENANCE JOBS (Always Run)
        # ============================================
        
        # Bot conversation state cleanup at 1:00 AM
        job_queue.run_daily(
            bot_conversation_cleanup,
            bot_cleanup_time,
            name="bot_cleanup"
        )
        logger.info("Bot conversation cleanup scheduled for 1:00 AM MT")
        
        # Daily reset at 11:00 PM
        job_queue.run_daily(
            daily_reset,
            reset_time,
            name="daily_reset"
        )
        logger.info("Daily reset scheduled for 11:00 PM MT - Forces ALL users to leave")
        
        # Daily snapshot at 4:45 PM
        job_queue.run_daily(
        ...
        # Hourly auto-checkout for stale check-ins
        ...
        # ============================================
        # 4:00 AM MORNING CONTENT (Always Run - Ignores Toggle)
        # ============================================
        
        # Weather update at 4:00 AM
        job_queue.run_daily(
        ...
        logger.info("Weather update scheduled for 4:00 AM MT (always runs)")
        ...
        logger.info("MOTD scheduled for 4:00 AM MT (always runs)")
        
        # ============================================
        # ACTIVE RIDERS ANNOUNCEMENTS (Respects Toggle)
        # ============================================
        
        # Active Riders at 4:30 AM
        job_queue.run_daily(
            auto_announce,
            announce_430am,
            data=GROUP_CHAT_ID,
            name="auto_announce_0430"
        )
        
        # Active Riders at 7:00 AM
        job_queue.run_daily(
            auto_announce,
            announce_7am,
            data=GROUP_CHAT_ID,
            name="auto_announce_0700"
        )
        logger.info("Active Riders scheduled for 4:30 AM and 7:00 AM MT (respects toggle)")
